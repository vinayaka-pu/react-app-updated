...
export const SearchBox = () => {
    ...
    const handleClick = (e) => {
        e.preventDefault();
        const url = `https://kcp1t1hcj5.execute-api.ap-south-1.amazonaws.com/live2/bucket?company_name=${searchTerm}`;
        fetch(url)
        .then(res => res.json())
        .then(data => {
                console.log(data)
            ...
            //     if(data[i].company_name === searchTerm){
                    setSearchAddress(data[0].location);
                    return;
            //     }
            // }
            // setSearchAddress(data[searchTerm].location);
            
            // setSearchAddress('Please enter a valid company name & also check uppercase-lowercase');
            setSearchAddress("Company doesn't exist");
        })
